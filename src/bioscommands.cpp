...
/*Start of exteded IPMI OEM command which uses the oem, IANA number format */
    ...
    ipmi_ret_t foxconnOemCount(const uint8_t* reqBuf,
    ...
    ipmi_ret_t handlebioscommand(ipmi_cmd_t, const uint8_t* reqBuf, uint8_t* replyBuf,
            size_t* dataLen)
    {
        if ((*dataLen) < 1)
        {
        ...
        if (reqBuf[0] == 0)
        {
            return foxconnOemCount(reqBuf, replyBuf, dataLen);
        }
    }

    void setupfoxconnOembiosCommands() __attribute__((constructor));
    void setupfoxconnOembiosCommands()
    {
