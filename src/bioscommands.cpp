        ...
        std::vector<uint8_t> reqParams)
{
    ...
    else if(reqParams.size() == 1)
    {
        ...
        if(boot_count_operation == BOOT_COUNT_READ &&
                boot.boot_count_header != INITIAL_BOOT_COUNT_HEADER)
        {
            boot.boot_count = INITIAL_VALUE;
        }
        ...
        {
            boot.boot_count_header = INITIAL_BOOT_COUNT_HEADER;
            boot.boot_count = START_BOOT_COUNT_VALUE;
        ...
        {
            boot.boot_count_header = DEFAULT_VALUE;
