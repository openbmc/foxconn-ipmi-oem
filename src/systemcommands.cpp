...
ipmi::RspType<uint8_t, uint8_t> FiiSysPCIeInfo(boost::asio::yield_context yield)
{
    ...
    std::sscanf(str.c_str(), "%x %x", bifurcationPresence, bifurcationPresence + 1);

    return ipmi::responseSuccess(bifurcationPresence[0], bifurcationPresence[1]);
}
